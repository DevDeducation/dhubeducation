<script lang="ts">
	import type { iPartner } from "$lib/interfaces";
	import Arrowlink from "../partials/Arrowlink.svelte";
  import { utilsstore } from "$lib/stores/utilsstore";
	import { imgparams } from "$lib/utils/index";

  export let partner: iPartner
	export let small: boolean = false;
  export let name: string = '';

	const cardName = partner.name.toLowerCase()

	const isSelected = cardName === name.toLowerCase()
</script>

{#if !small}
<a
  href={`/partners/${partner.slug}`}
  rel="prefetch"
  aria-label={partner.name}
  class="flex flex-col bg-white dark:bg-dark-800 rounded-lg overflow-hidden shadow-custom">
  <div class="grid grid-cols-1 gap-2">
    <img src={`${partner.landscapepic}${imgparams}`} class="w-full object-cover" alt={partner.name} />
    <div class="p-4 flex flex-col items-center gap-2">
      <h2 class="one-line w-full sub-title">
        {partner.name}
      </h2>
      <p class="two-lines">{ partner.content }</p>
      <div aria-label="details" class="flex flex-col gap-2">
        <hr class="dark:opacity-30" />
        <Arrowlink title="learn more" />
      </div>
    </div>
  </div>
</a>
{:else}
<a
href={`/partners/${partner.slug}`}
rel="prefetch"
aria-label={partner.name}
class={"flex flex-col rounded-lg overflow-hidden shadow-custom bg-white dark:bg-dark-800 p-2 gap-2"}>
<div class="grid grid-cols-[1fr_2fr] gap-2">
  <img src={`${partner.squarepic}${imgparams}`} class={"object-cover rounded-md"} alt={partner.name}>
  <div class="w-full h-full overflow-hidden text-ellipsis flex flex-col justify-between">
    <div class="flex flex-col justify-between h-full text-left">
      <div>
        <h2 class="text-base font-semibold one-line w-full overflow-hidden text-ellipsis">
          {partner.name}
        </h2>
        <p class="two-lines dark:opacity-60 w-full">
          {partner.country}
        </p>
      </div>
      <div class="flex items-end justify-between gap-2">
        <div class="h-[40px] dark:bg-white dark:p-2 rounded-lg">
          <img src={partner.logo} class="h-full dark:brightness-1 dark:fill-white text-white object-contain" alt={partner.name}/>
        </div>
        <img src={$utilsstore.flag(partner.countrycode)} class="w-12" alt={partner.name}/>
      </div>
      {#if isSelected}
         <div class="rounded-[50px] py-1 px-2 w-fit uppercase bg-primary text-white dark:bg-white dark:text-primary live text-2xs font-semibold">viewing</div>
      {/if}
    </div>
  </div>
</div> 
<div aria-label="details" class="flex flex-col gap-2">
  <hr class="dark:opacity-30" />
  <Arrowlink title="learn more" />
</div>
</a>
{/if}
