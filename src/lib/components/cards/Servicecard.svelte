<script lang="ts">
	import type { TService, iService } from '$lib/interfaces';
	...
	const cardName = service.name.toLowerCase() as TService;
...
{#if small}
	<a
		...
		<div class="grid grid-cols-[1fr_2fr] gap-2">
			...
			<img
				src={`${service.squarepic}${imgparams}`}
			...
			<div class="w-full h-full overflow-hidden text-ellipsis flex flex-col justify-between">
				<div class="flex flex-col justify-between h-full">
					...
					{#if isSelected}
						<div
							class="rounded-[50px] py-1 px-2 w-fit uppercase bg-primary text-white dark:bg-white dark:text-primary live text-2xs font-semibold">
							viewing
						</div>
					{/if}
				</div>
...
{:else}
	<a
		...
		<div class="grid grid-cols-1 gap-2">
			<img
				src={`${service.landscapepic}${imgparams}`}
