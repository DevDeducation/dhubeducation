<script lang="ts">
	...
	import { imgparams, utils } from "$lib/utils/index";
...
<a
  ...
  <div class="grid grid-cols-1 gap-2">
    <img src={`${blog.banner}${imgparams}`} class="w-full object-cover" alt={blog.title} />
    <div class="p-4 flex flex-col items-center gap-4">
      <div>
        <h2 class="one-line w-full sub-title">
          {blog.title}
        </h2>
        <p class="two-lines min-h-12">{ blog.caption }</p>
      </div>
      <hr class="dark:opacity-30 w-full" />
      <div aria-label="details" class="flex items-center justify-between w-full">
        <p class="text-xs font-bold">{utils.formatDate(new Date(blog.date))}</p>
...
<div class="grid grid-cols-[1fr_2fr] gap-2">
  <img src={`${blog.thumbnail}${imgparams}`} class={"object-cover rounded-md"} alt={blog.title}>
  <div class="w-full h-full overflow-hidden text-ellipsis flex flex-col justify-between">
    <div class="flex flex-col justify-between h-full">
      ...
      <div class="flex gap-2 justify-between items-center">
        <p class="text-xs font-bold">{utils.formatDate(new Date(blog.date))}</p>
        <div class="rounded-[50px] py-1 px-2 w-fit uppercase bg-primary text-white dark:bg-white dark:text-primary text-2xs font-semibold">{blog.category}</div>        
        {#if isSelected}
